...
(defun time-to-epoch (&optional time)
  ...
  (truncate (time-to-seconds (date-to-time time))))
...
(defun epoch-to-time (&optional epoch)
  ...
  (interactive)
  (unless epoch
    (setq epoch (truncate (time-to-seconds (current-time)))))
  (when (stringp epoch)
  ...
  (cond
   ...
   ((numberp time)
    (if (> time 0)
      ...
      (+ (truncate (time-to-seconds current-time)) time)))
...
;; epoch menu buffer name
(defvar epoch-menu-buffer-name
  "*Epoch Time Conversion*"
  "Buffer name to use for epoch menu interface.")

;; epoch interface
;;;###autoload
(defun epoch (&optional times)
...
\nIf TIMES is non-nil, it adds them to the examples list."
  ...
  (let (buffer                           ; menu buffer
        ...
        last-time)                       ; last time
    ;; setup buffer
    (when (get-buffer epoch-menu-buffer-name)
      (kill-buffer epoch-menu-buffer-name))
    (setq buffer (get-buffer-create epoch-menu-buffer-name))
    (set-buffer buffer)
    ;; (setq buffer-read-only nil)
    ;; (erase-buffer)
    (kill-all-local-variables)
    ;; use local variable to store time fields
    ...
    ;; last time
    (setq last-time (or (car times) (car epoch-time-examples) (truncate (time-to-seconds (current-time)))))
    ;; add header
          ...
          (widget-create
           ...
           :notify (lambda (widget &rest ignore)
                     (let* ((value (widget-value widget))
                            (time-string (condition-case nil (epoch-to-time (string-to-number value))
                                           (error ""))))
    ...
    (setq widget-time-string
          (widget-create
           ...
           :notify (lambda (widget &rest ignore)
                     (let* ((value (widget-value widget))
                            (time-epoch (condition-case nil (float-to-string (time-to-epoch value))
                                          (error ""))))
    ...
    (widget-create 'push-button
                   ...
                   :notify `(lambda (&rest ignore)
                              (let ((time-epoch (widget-value widget-time-epoch)))
                                (pushnew (string-to-number time-epoch) epoch-time-examples)
                                (kill-buffer nil)
                                (epoch-menu ,times))))
    (widget-insert "\n\n")
    ;; times selection
    ...
    (do ((time-examples time-examples (cdr time-examples))
         (cnt 1 (1+ cnt)))
        ((not time-examples))
