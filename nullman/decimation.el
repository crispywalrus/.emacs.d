...
\nColor properties should already be applied to CHAR."
  (let* ((inhibit-read-only t))
    (goto-char (aref grid-buffer-pos pos))
    (delete-char 1)
    (insert char)
    (goto-char (point-max))))
...
If CAN-SEE is t, the monster's color is changed."
  (let (new-monsters
    ...
    (setq monsters (nreverse new-monsters))))
...
(defun decimation-kill-monster (monster)
  ...
  (let* ((rationality (/ (+ (first hero) hero-max-rationality) 2))
         (increase (- rationality (first hero))))
    (setq hero (list rationality (second hero)))
...
MONSTERS is the number of random monsters to create, defaults to `decimation-monsters'.
RATIONALITY is the hero's maximum hit points, defaults to `decimation-rationality'."
  (let* (grid-free)
    (setq
     ...
     grid (make-vector grid-size decimation-grid-floor-char)
     ;; clear monsters
     monsters nil
     ;; clear hero
     hero nil
    ...
    (when (< grid-size (1+ monster-count))
    ...
    (dotimes (x monster-count)
      (let ((type (1+ (mod x 9)))
        ...
        (push (list x type pos) monsters)
        ;; put monster in grid
        (aset grid pos (decimation-digit-to-char type))
        ;; remove grid tile
        (delq pos grid-free)))
    (setq monsters (nreverse monsters))
    ;; place hero
    (let ((pos (nth (random (length grid-free)) grid-free)))
      ;; set hero info
      (setq hero (list rationality pos))
      ;; put hero in grid
      (aset grid pos decimation-grid-hero-char))))

;; move hero
(defun decimation-hero-move (dx dy)
  "Move hero DX in the X axis and DY in the Y axis."
  (let* ((hero-pos (second hero))
    ...
    (when new-pos
      (cond
       ...
       ((not (decimation-grid-pos-in-bounds new-pos grid))
        (message "Cannot move hero out of bounds"))
       ;; cannot move onto monster tile
       ((member new-pos (mapcar 'third monsters))
        (message "Cannot move hero onto a monster"))
       ;; move hero
       (t
        (aset grid hero-pos decimation-grid-floor-char)
        (aset grid new-pos decimation-grid-hero-char)
      ...
      (setq hero (list (first hero) new-pos))
      ;; move monsters
...
(defun decimation-move-monsters ()
  ...
  (let ((move hero-moves))
    (while (and (> move 0)
      ...
      (setq move (1- move))
      (let (moving-monsters
            (hero-pos (second hero)))
        ;; determine which monsters move this turn
        (dolist (monster monsters)
          (when (= (mod turn (second monster)) 0)
          ...
          (let ((x (first (decimation-grid-pos-to-xy (third monster))))
            ...
            (if (decimation-monster-can-see monster hero-pos)
              ...
              (let (dx
                    ...
                    pos)
                (while (or (not pos)
                           (not (decimation-grid-pos-in-bounds pos grid))
                           (and (= dx 0) (= dy 0)))
                  (setq dx (1- (random 3))
                        dy (1- (random 3))
  ...
  (cond
   ...
   ((= (second hero) pos)
    ...
    (let* ((type (second monster))
      ...
      (setq hero (list (- (first hero) damage) (second hero)))))
   ;; move monster
   ((not (member pos (mapcar 'third monsters)))
...
(defun decimation-hero-attack (funct)
  ...
  (when (decimation-hero-status)
    (let ((monster-targets (decimation-monsters-next-to-hero monsters (second hero))))
...
(defun decimation-draw-screen ()
  ...
  (setq grid-buffer-pos (make-vector grid-size nil))
  (let ((inhibit-read-only t))
    (erase-buffer)
    (dotimes (x grid-offset-height)
      (newline))
    (dotimes (y grid-height)
      ...
      (dotimes (x grid-width)
        (let* ((pos (+ (* y grid-width) x))
               (char (aref grid pos)))
          (aset grid-buffer-pos pos (point))
          (cond
  ...
  (dolist (monster monsters)
    (when (decimation-monster-can-see monster (second hero))
      (decimation-set-monster monster (third monster) nil t))))
...
(defun decimation ()
  ...
  (setq
   ...
   grid-buffer-pos nil
   ;; grid width
   grid-width decimation-grid-width
   ;; grid height
   grid-height decimation-grid-height
   ;; monsters info
   monsters nil
   ;; hero info
   hero nil
   ...
   hero-max-rationality decimation-hero-rationality
   ;; turn
   turn 0
   ;; whitespace between cells
   grid-space (if (and (>= (window-width) (1- (* grid-width 2)))
                       (>= (window-height) (1- (* grid-height 2))))
                  2
                1)
   ;; left margin
   grid-offset-width (if (= grid-space 2)
                         (/ (- (1- (window-width)) (1- (* grid-width 2))) 2)
                       (/ (- (1- (window-width)) grid-width) 2))
   ;; right margin
   grid-offset-height (if (= grid-space 2)
                          (/ (- (1- (window-height)) (1- (* grid-height 2))) 2)
                        (/ (- (1- (window-height)) grid-height) 2)))
   ;; initialize
  (decimation-initialize-grid grid-width grid-height decimation-monsters
                              hero-max-rationality decimation-hero-moves)
  ;;(message "grid: %s, monsters: %s, hero: %s" grid monsters hero)
  ;; draw screen
